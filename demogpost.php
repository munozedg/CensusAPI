...
if (isset($_POST['field'])){$field = make_safe($_POST['field']);} //comma delimited list
if (isset($_POST['state'])){$state = make_safe($_POST['state']);}
if (isset($_POST['county'])){$county = make_safe($_POST['county']);}
if (isset($_POST['geonum'])){$geonum = make_safe($_POST['geonum']);} //comma delimited list
if (isset($_POST['geoid'])){$geoid = make_safe($_POST['geoid']);}
if (isset($_POST['sumlev'])){$sumlev = make_safe($_POST['sumlev']);}
if (isset($_POST['table'])){$table = make_safe($_POST['table']);} //comma delimited list

//potential single select
if (isset($_POST['type'])){$type = make_safe($_POST['type']);}else{$type='json';} //if no db given, assume most current
if (isset($_POST['db'])){$db = make_safe($_POST['db']);}else{$db='acs0913';} //if no db given, assume most current
//set default for schema if it is missing
if (isset($_POST['schema'])){$schema = make_safe($_POST['schema']);}else{
...
if (isset($_POST['geo'])){$geo = make_safe($_POST['geo']);}
if (isset($_POST['series'])){$series = make_safe($_POST['series']);}
if (isset($_POST['type'])){$type = make_safe($_POST['type']);}

  $limit=1000;  //by default limits to 1000 search results.  POST 
if (isset($_POST['limit'])){$limit = make_safe($_POST['limit']);}

//if database is acs0812, check to see if moe option is flagged
$moe='no';
if($db=='acs0812' or $db=='acs0913'){if (isset($_POST['moe'])){$moe=make_safe($_POST['moe']);}}
